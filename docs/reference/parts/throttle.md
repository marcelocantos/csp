...
|-----------|------|---------|-------------|
| `d` | `csp::clock::duration` | | Interval between budget resets |
| `n` | `size_t` | `1` | Number of values allowed per interval |
...
- An internal `tick(d)` timer resets the remaining budget to `n` each
  interval.
...
- **Values arrive faster than the budget allows:** excess values within each
  interval are dropped. The first `n` values per interval always pass.
...
auto r = throttle<int>(1s).spawn(source.spawn());
```
...
auto r = throttle<int>(100ms, 5).spawn(source.spawn());
```

### Composed with `|`

```cpp
auto pipeline = count(1, 6) | throttle<int>(1s, 2);
...
```cpp
auto ch = throttle<int>(100ms, 2).spawn();
