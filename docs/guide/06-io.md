...
A pipe-based echo that reads lines, transforms them to uppercase, and writes
them back:
...
int main() {
    ...
    // Read side: fd → bytes → lines
    auto lines_in = csp::part::lines().spawn(
                        csp::part::byte_reader(pipefd[0]).spawn());

    // Write side
    auto bytes_out = csp::part::byte_writer(pipefd[1]).spawn();

    // Producer: send some lines
    ...
    // Consumer: uppercase each line, write back
