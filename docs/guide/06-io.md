...
int main() {
    ...
    // Read side: input pipe → bytes → split_lines
    auto lines_in = csp::part::io::split_lines.spawn(
                        csp::part::io::byte_reader(in_pipe[0]).spawn());

    // Write side: → bytes → output pipe
    auto bytes_out = csp::part::io::byte_writer(out_pipe[1]).spawn();
