...
auto lr = csp::part::split_lines().spawn(std::move(r));
```
...
```cpp
auto fr = csp::part::fixed_frames(512).spawn(std::move(r));
```
...
auto lr = csp::part::split_lines().spawn(
...
```cpp
auto pipeline = csp::part::byte_reader(fd) | csp::part::split_lines();
...
int main() {
    ...
    auto lines_in = csp::part::split_lines().spawn(
