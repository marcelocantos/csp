...
## Tier A — Fills clear gaps in the algebra

- [x] **share** `(reader<T> source) → reader<reader<T>>` — Already implemented
      in `include/csp/part/share.h`.

- [x] **switch_map** `(reader<A>, F(A)→reader<B>) → reader<B>` — Latest-wins
      flat_map: each new input cancels the previous sub-stream. Cancellation
      via endpoint lifecycle (drop previous reader). Implemented as
      `switch_all<B>` (+ `map | switch_all` composition).

- [x] **conflate** `(reader<T>, F(T,T)→T) → reader<T>` — When downstream is
      slow, merge pending upstream values via combining function. No buffering,
      no dropping. Uses alt to race input vs output readiness.

- [x] **combine_latest** `(reader<A>, reader<B>, ...) → reader<tuple<A,B,...>>`
      — Emits whenever any input updates, using the latest value from each.
      Foundation for reactive UIs, sensor fusion, etc. Implemented in
      `include/csp/part/combine_latest.h`. Also supports a combining function
      overload: `combine_latest<A, B>(ra, rb, f)`.

- [x] **parallel_map** `(size_t n, F(A)→B, {.ordered}) → filter<A,B>` —
      Concurrent transform with demand-driven dispatch. `ordered=true`
      reassembles in input order via reorder buffer; `ordered=false` (default)
      emits as completed. Implemented in `include/csp/part/parallel_map.h`.

- [x] **pace** `(reader<Trigger>) → filter<T,T>` — Rate-limited passthrough
      preserving all values via backpressure (unlike throttle which drops).
      Takes an external trigger channel (consistent with throttle). Use with
      `tick(d)` for periodic pacing. Implemented in `include/csp/part/pace.h`.

- [x] **multiplex** `(reader<Ts>...) → reader<variant<Ts...>>` / **demux**
      `(reader<variant<Ts...>>) → tuple<reader<Ts>...>` — Heterogeneous typed
      merge/split via variant. The typed `select()` loop. Implemented as
      `mux` and `demux` in `include/csp/part/mux.h` and `demux.h`.

- [x] **try_map** `(F, writer<exception_ptr>) → filter<A, B>` — Per-element
      error handling: if F throws, `exception_ptr` is sent to error channel and
      input value is dropped. Pipeline continues with next element. Dead error
      channel only stops processing when an error actually occurs. Without an
      error channel, delegates to `map`. Implemented in
      `include/csp/part/try_map.h`.

## Tier B — Very useful building blocks

- [x] **concat_map** `(reader<A>, F(A)→reader<B>) → reader<B>` — Sequential
      flat_map: each sub-stream runs to completion before the next starts.
      Implemented as `concat_all<B>` (+ `map | concat_all` composition).

- [x] **exhaust_map** `(reader<A>, F(A)→reader<B>) → reader<B>` — Ignores new
      inputs while current sub-stream is active. Prevents duplicate work.
