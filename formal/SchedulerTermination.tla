 ...
 * has_global_work) BEFORE decrementing pending_signals (outside
 * signal_mu).  If the scheduler drains the imp between the push and
 * decrement, has_global_work is cleared.  When a second signal then
 * pushes AND decrements (pending_signals hits 0), the scheduler sees
 * pending_signals = 0 and must ALSO check has_global_work to avoid
 * missing the queued imp.
 *
 * Fix: the exit condition checks BOTH pending_signals = 0 AND
 * has_global_work = false.  Since push_to_global sets has_global_work
 * before any decrement, there is no window where both are zero while
 * an imp is on the global queue.
 *
 * Participants:
 ...
 *
 * TLA:SchedulerTermination.ReactorPush *)
ReactorPush(s) ==
    /\ sig_state[s] = "pending"
 ...
 *
 * TLA:SchedulerTermination.ReactorDecr *)
ReactorDecr(s) ==
    /\ sig_state[s] = "pushed"
 ...
 *       if (run()) continue;
 *       if (live_gs == 0) break;
 *       if (!mn_mode_ && !has_pending_signals()
 *           && !has_global_work) break;              // FIX
 *       park on CV;
 *   }
 *
 ...
 *
 * TLA:SchedulerTermination.SchedDrain *)
SchedDrain ==
    /\ sched_pc = "drain"
 ...
 *
 * TLA:SchedulerTermination.SchedRunOne *)
SchedRunOne ==
    /\ sched_pc = "run_one"
    /\ local_count > 0
...
(* Exit check.  FIX: checks BOTH pending_signals AND has_global_work.
 *
 * Code (fixed): csp.cc:19,27-29
 *   if (live_gs == 0) break;
 *   if (!mn_mode_
 *       && !has_pending_signals()
 *       && !has_global_work_.load(acquire)) break;
 *   park;
 *
 * TLA:SchedulerTermination.SchedCheckExit *)
SchedCheckExit ==
    /\ sched_pc = "check_exit"
    /\ IF live_gs = 0
       THEN sched_pc' = "exited"
       ELSE IF pending_signals = 0 /\ ~has_global_work
            THEN sched_pc' = "exited"          \* FIX: both must hold
 ...
 *   });
 *
 * TLA:SchedulerTermination.SchedWake *)
SchedWake ==
    /\ sched_pc = "parked"
    /\ \/ live_gs = 0
       \/ has_global_work
